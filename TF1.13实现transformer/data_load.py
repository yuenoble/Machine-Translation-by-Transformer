...
def get_batch_data():
    ...
    '''tf.train.slice_input_producer对训练数据进行处理，该函数从输入中每次取一个切片返回到一个输入队列里，
    该队列作为之后tf.train.shuffle_batch的一个参数，用以生成一个batch的数据。'''
    # create batch queues
    ...
    return x, y, num_batch  #  shape=[batch_size, max_seq_len]